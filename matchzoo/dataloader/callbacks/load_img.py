...
class LoadImage(BaseCallback):
    ...
    def for_list_unpacked(self, left, right):
        image, image_length = self.load_images(left['id_left'], 1)
        left['image_left'] = image
        left['image_left_length'] = image_length

        right['image_right'] = []
        right['image_right_length'] = []

        for i in range(len(right['id_right'])):
            id_right = right['id_right'][i]
            image, image_length = self.load_images([id_right], 1)
            right['image_right'].append(image[0])
            right['image_right_length'].append(image_length[0])