...
class TextFieldPadding(BasicPadding):
    ...
    def for_list_on_batch_unpacked(self, left, right):
        for field, fixed_pad_length, pad_value in self._pad_args:
            if field in left:
                batch_size = len(left[field])

                length_field = field + '_length'
                if fixed_pad_length is not None:
                    pad_length = fixed_pad_length
                else:
                    pad_length = max(min(int(max(left[length_field])), self._max_length), self._min_length)
            else:
                batch_size = len(right[field])

                length_field = field + '_length'
                if fixed_pad_length is not None:
                    pad_length = fixed_pad_length
                else:
                    pad_length = max(min(int(max(right[length_field])), self._max_length), self._min_length)
            ...
            if field in left:
                ...
                padded_value = np.full([batch_size, pad_length], pad_value, dtype=dtype)
                _padding_2D(value, padded_value, self._pad_word_mode)
                left[field] = padded_value

                length = np.array(left[length_field])
                left[length_field] = np.where(length > pad_length, pad_length, length)
            else:
                value = right[field]
                dtype = _infer_dtype(value)
                padded_value = np.full([batch_size, pad_length], pad_value, dtype=dtype)
                _padding_2D(value, padded_value, self._pad_word_mode)
                right[field] = padded_value

                length = np.array(right[length_field])
                right[length_field] = np.where(length > pad_length, pad_length, length)                
...
class ImageFieldPadding(BasicPadding):
    ...
    def for_list_on_batch_unpacked(self, left, right):
        for field in self._image_fields:
            ...
            if field in left:
                padded_length = max(min(int(max(left[length_field])), self._max_length), self._min_length)
                if self._fixed_length is not None:
                    padded_length = self._fixed_length
                ...
                # add to x
                left[field] = np.stack(field_feat, axis=0)
                length = np.array(left[length_field])
                left[length_field] = np.where(length > padded_length, padded_length, length)
            else:
                padded_length = max(min(int(max(right[length_field])), self._max_length), self._min_length)
                if self._fixed_length is not None:
                    padded_length = self._fixed_length
                
                for v in right[field]:
                    value = self._pad_each_item(v, padded_length)
                    field_feat.append(value)

                # add to x
                right[field] = np.stack(field_feat, axis=0)
                length = np.array(right[length_field])
                right[length_field] = np.where(length > padded_length, padded_length, length)