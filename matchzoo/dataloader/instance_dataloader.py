...
class InstanceDataLoader(DataLoader):
    ...
    def build_dataloader(self):
        if self._if_list:
            self._dataloader = data.DataLoader(
                ...
                shuffle=True,
                collate_fn=self._list_collate_fn,
                drop_last=True,
    ...
    def __iter__(self) -> typing.Tuple[dict, torch.tensor]:
        ...
        if self._if_list:
            for batch_data in self._dataloader:
                left, right = batch_data
                for callback in self._callbacks:
                    callback.for_list_on_batch_unpacked(left, right)
                ...
                def to_tensor_dict(x):
                    ...
                    for key, value in x.items():
                        ...
                        if 'image_left' in key or 'image_right' in key or 'label' in key:
                            torch_value = torch_value.float()
                        
                        if 'right_list_valid_mask' in key:
                            torch_value = torch_value.bool()
                        
                        tensor_dict_x[key] = torch_value

                    return tensor_dict_x
                
                yield to_tensor_dict(left), to_tensor_dict(right)
        else:
            for batch_data in self._dataloader:
                x, y = batch_data
    ...
    def _list_collate_fn(self, instances):
        left_zip, right_zip = unzip(instances)
        left_list = list(left_zip)
        right_list = list(right_zip)

        gather_left = {}
        gather_right = {}
        dict_keys_left = left_list[0].keys()
        dict_keys_right = right_list[0].keys()

        for key in dict_keys_left:
            try:
                gather_left[key] = list(concat(i[key] for i in left_list))
            except:
                print(key, 'l!')

        for key in dict_keys_right:
            try:
                gather_right[key] = list(concat(i[key] for i in right_list))
            except:
                print(key, 'r!')

        return gather_left, gather_right