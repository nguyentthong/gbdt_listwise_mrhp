...
class ModelingTrainer(Trainer):
    ...
    def __init__(
        ...
        debug: bool = False,
        config=None,
        train_mode: str = 'list_train',
        test_mode: str = 'list_test',
        **kwargs
    ):
        ...
        self.pairwise_train_losses = []
        self.listwise_train_losses = []
    ...
    def run(self):
        ...
        for epoch in range(self._start_epoch, self._epochs + 1):
            ...
            if self._train_mode == 'list_train':
                self._run_epoch_list_mode()
            else:
                self._run_epoch()
            if self._check_early_stopping():
                break

        with open(os.path.join(str(self._save_dir), 'listwise_train_losses.txt'), 'w') as f:
            for i in range(len(self.listwise_train_losses)):
                listwise_train_loss = self.listwise_train_losses[i]
                f.write(str(listwise_train_loss))
                f.write('\n')

        with open(os.path.join(str(self._save_dir), 'pairwise_train_losses.txt'), 'w') as f:
            for i in range(len(self.pairwise_train_losses)):
                pairwise_train_loss = self.pairwise_train_losses[i]
                f.write(str(pairwise_train_loss))
                f.write('\n')
        if self._verbose:
    ...
    def _run_epoch(self):
        ...
        with tqdm(enumerate(self._trainloader), total=num_batch,
            ...
            for step, (inputs, target) in pbar:
                ...
                outputs, pairwise_outputs, listwise_outputs = self._model(inputs, target)
                loss = torch.sum(
                    *[c(outputs, target) for c in self._criterions]
                )
                # neural NDCG
                from allrank.models.losses import neuralNDCG
                neuralNDCG_loss = neuralNDCG(target.reshape(-1,self._config.train.loss.num_neg+1), listwise_outputs.reshape(-1,self._config.train.loss.num_neg+1))
                total_loss = neuralNDCG_loss + loss

                # listnet
                # import torch.nn.functional as F
                # list_outputs = outputs.reshape(-1, 8)
                # list_target = target.reshape(-1, 8)
                # dist_outputs = F.softmax(list_outputs)
                # dist_target = F.softmax(list_target)
                # ce_loss = -torch.sum(dist_target * torch.log(dist_outputs))
                # total_loss = loss + self._config.train.alpha*ce_loss

                # MSE
                # total_loss = loss + 0.25*mse_loss

                # calculate all losses and sum them up
                self._backward(total_loss)
                train_loss.update(total_loss.detach().cpu().item())
    ...
    def _run_epoch_list_mode(self):
        num_batch = len(self._trainloader)
        train_loss = AverageMeter()

        with tqdm(enumerate(self._trainloader), total=num_batch, disable=not self._verbose) as pbar:
            for step, (left, right) in pbar:
                left = self._load_tensor(left)
                right = self._load_tensor(right)

                outputs, pairwise_outputs, listwise_outputs = self._model((left, right), labels=right['label'], data_mode='list')
                loss = torch.sum(
                    *[c(outputs, right['label']) for c in self._criterions]
                )
                # neural NDCG
                from allrank.models.losses import neuralNDCG, listNet
                if self._config.train.neural_ndcg_only: 
                    neuralNDCG_loss = neuralNDCG(right['label'].reshape(-1,self._config.train.loss.num_neg+1), listwise_outputs.reshape(-1,self._config.train.loss.num_neg+1))
                    total_loss = neuralNDCG_loss
                elif self._config.train.list_net_only:
                    listNet_loss = listNet(right['label'].reshape(-1,self._config.train.loss.num_neg+1), listwise_outputs.reshape(-1,self._config.train.loss.num_neg+1))
                    total_loss = listNet_loss
                elif self._config.train.sdt_score_only:
                    total_loss = loss - penalty*10
                else:
                    neuralNDCG_loss = neuralNDCG(right['label'].reshape(-1,self._config.train.loss.num_neg+1), outputs.reshape(-1,self._config.train.loss.num_neg+1))
                    listNet_loss = listNet(right['label'].reshape(-1,self._config.train.loss.num_neg+1), outputs.reshape(-1,self._config.train.loss.num_neg+1))
                    total_loss = listNet_loss + loss
                    # total_loss = neuralNDCG_loss + loss

                # calculate all losses and sum them up
                listwise_labels = right['label'].reshape(-1,self._config.train.loss.num_neg+1)
                self.listwise_train_losses.append(total_loss.detach().cpu().item()/listwise_labels.shape[0])
                self.pairwise_train_losses.append(loss.detach().cpu().item())
                self._backward(total_loss)
                train_loss.update(total_loss.detach().cpu().item())
    ...
    def predict(
        ...
        with torch.no_grad():
            ...
            listwise_predictions = []
            y_trues = []
            id_lefts = []
            id_rights = []
            val_loss_list = []
            if self._test_mode == 'list_test':
                for i, (left, right) in enumerate(tqdm(dataloader)):
                    left = self._load_tensor(left)
                    right = self._load_tensor(right)
                    outputs, _, _ = self._model((left, right), stage='test', data_mode='list')
                    y_trues.append(right['label'].detach().cpu().squeeze(-1))

                    outputs = outputs.detach().cpu()
                    predictions.append(outputs)

                    list_len = len(right['id_right']) // len(left['id_left'])
                    for id_left in left['id_left']:
                        for _ in range(list_len):
                            id_lefts.append(id_left)
                    
                    for id_right in right['id_right']:
                        id_rights.append(id_right)

                    torch.cuda.empty_cache()
                    # try:
                    #     from allrank.models.losses import neuralNDCG, listNet
                    #     val_loss = listNet(right['label'].reshape(1, -1).cpu(), outputs.reshape(1, -1))
                ...
                for inputs, _ in tqdm(dataloader):
                    ...
                    outputs, pairwise_outputs, listwise_outputs = self._model(inputs, stage='test')
                    outputs = outputs.detach().cpu()
                    pairwise_outputs = pairwise_outputs.detach().cpu()
                    listwise_outputs = listwise_outputs.detach().cpu()
                    predictions.append(outputs)
                    pairwise_predictions.append(pairwise_outputs)
                    listwise_predictions.append(listwise_outputs)
            self._model.train()
            # self.validation_losses.append(np.mean(val_loss_list))
            return id_lefts, id_rights, torch.cat(y_trues, dim=0).reshape(-1).numpy(), torch.cat(predictions, dim=0).numpy()
    ...
    def evaluate(
        ...
        listwise_result = dict()
        id_left, id_right, y_true, y_pred = self.predict(dataloader)
        ...
        if isinstance(self._task, tasks.Classification):
            ...
            for metric in self._task.metrics:
                result[metric] = self._eval_metric_on_data_frame(metric, id_left, y_true, y_pred.squeeze(axis=-1))
        return result
