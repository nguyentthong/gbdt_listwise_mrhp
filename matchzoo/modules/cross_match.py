...
class BertCrossMatchLayer(nn.Module):
    def __init__(
                self, 
                do_normalize: bool = True,
                num_repeat: int = 9):
    ...
    def forward(self,
        ...
        if self.do_normalize:
            ...
            input_right = F.normalize(input_right, p=2, dim=-1)

        # masking
        mask = torch.matmul(
            input_left_unpadding_mask.unsqueeze(-1).float(),
            input_right_unpadding_mask.unsqueeze(1).float()
        )

        cross_match_sim = torch.matmul(input_left, input_right.transpose(1, 2)).unsqueeze(1).repeat(1, self.num_repeat, 1, 1)
        cross_match = cross_match_sim * mask.unsqueeze(1)
        return cross_match